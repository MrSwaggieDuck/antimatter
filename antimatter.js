...
//Prestige
	...
	if(dimensionBoost.amount >= 10) {
		document.querySelector("#galaxyButton").innerHTML = "Start a new galaxy for " + (dimensionBoost.amount-9) + " Matter";
	} else {
...
function dimensionBoostF() {
	if(dimension8.amount >= dimensionBoost.cost) {
		if(dimensionBoost.amount < 9) {
			dimensionBoost.cost += 10;
		} else {
			dimensionBoost.cost *= 1.2;
		}
...
function newGalaxy() {
	if(dimensionBoost.amount >= 10) {
		matter += dimensionBoost.amount - 9;
		antimatter = 10;
		dimensionBoost = {
			cost:10,
			multiplier:1,
